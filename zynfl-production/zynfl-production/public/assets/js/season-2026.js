...
function zPhoto(src,name,cls=''){return `<span class="z-photo ${cls}"><span aria-hidden="true">${zEsc(zInitials(name))}</span>${src?`<img src="${zEsc(src)}" alt="" loading="lazy">`:''}</span>`;}
function zWireImages(root=app){root.querySelectorAll('.z-photo img').forEach(img=>{img.addEventListener('error',()=>img.remove(),{once:true});});}
...
function zFormation(rows,top,nfl){
 ...
 return `<div class="z-formation ${top?'z-top':'z-bottom'}"><div class="z-backs">${backs.map(p=>zBubble(p,nfl)).join('')}</div><div class="z-qb">${qb?zBubble(qb,nfl):'<span>QB unavailable</span>'}</div><div class="z-receivers">${wings.map(p=>zBubble(p,nfl)).join('')}</div>${zLine()}</div>`;
}
function zSideHeader(key,score,nfl,bottom=false){
 ...
 return `<div class="z-team-header ${bottom?'z-header-bottom':''}">${zPhoto(photo,title)}<div><span class="z-eyebrow">${nfl?key:key==='Chuck'?'Charlie (Chuck)':key==='Santi'?'Santiago (Santi)':key} ${nfl?'':'· '+zRecord(key)}</span><h2>${zEsc(title)}</h2></div><div class="z-team-score"><strong>${zPts(score)}</strong><small>${nfl?'NFL final':'Week 1 points'}</small></div></div>`;
}
...
function zDrawMatchups(){
 ...
 list.innerHTML=games.map((g,i)=>`<button class="z-match-choice ${zState.match===i?'selected':''}" data-match="${i}" aria-pressed="${zState.match===i}"><small>WEEK 1 · FINAL</small>${[g[0],g[1]].map((key,j)=>`<span><b>${zEsc(nfl?key:key==='Chuck'?'Charlie':key==='Santi'?'Santi':key)}</b><strong>${zPts(nfl?g[j+2]:zTotal(key))}</strong></span>`).join('')}</button>`).join('');
...
function zJackOrder(){try{const v=JSON.parse(localStorage.getItem('zynfl-jack-rankings-2026-w1'));if(zValidOrder(v))return v;}catch{}return zValidOrder(Z26.jackRankings['1'])?Z26.jackRankings['1']:null;}
...
function zDrawRankings(){
 ...
 document.getElementById('zRankingContent').innerHTML=order?`${jack?'<p class="z-muted">Jack’s power rankings based on my opinion. Local edits are visible only in this browser until exported and added to the site data.</p>':''}<ol class="z-ranking-list">${order.map((o,i)=>`<li><strong class="z-rank">${String(i+1).padStart(2,'0')}</strong>${zPhoto(teamPhoto(o),o)}<div><h3>${zEsc(Z26.teams[o].name)}</h3><small>${zEsc(o)} · ${zRecord(o)} · ${zPts(zTotal(o))} Week 1 points</small>${jack?'':`<p>${zEsc(Z26.rankReasons[o])}</p>`}${zScheduleNote(o)?`<p class="z-muted">${zEsc(zScheduleNote(o))}</p>`:''}</div></li>`).join('')}</ol>`:'<div class="z-empty"><h3>Jack’s rankings are coming.</h3><p>The Week 1 order has not been supplied yet. Use “Edit Jack’s rankings” to enter it.</p></div>';
...
function zEditJack(){
 ...
 d.querySelector('#zSaveRank').onclick=()=>{const a=read();if(!a)return;try{localStorage.setItem('zynfl-jack-rankings-2026-w1',JSON.stringify(a));}catch{d.querySelector('#zRankError').textContent='Browser storage is unavailable. Export JSON instead.';return;}d.close();zState.rankView='jack';zDrawRankings();};
 ...
 d.querySelector('#zResetRank').onclick=()=>{try{localStorage.removeItem('zynfl-jack-rankings-2026-w1');}catch{}d.close();zDrawRankings();};d.showModal();
}
