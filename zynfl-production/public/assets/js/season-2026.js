...
function zWeekButtons(){return `<div class="z-weekbar" aria-label="Season week"><span>2026</span><button aria-pressed="true" class="active">Week 1</button></div>`;}
function zBubble(p,nfl=false,compact=false){
 ...
 return `<button class="z-player ${compact?'z-compact':''}" data-z-player="${zEsc(p.id)}" style="--bubble:${size}px;--owner:${owner?.color||'#8ca0b6'}" aria-label="Open ${zEsc(p.name)} player details">${zPhoto(p.headshot,p.name,'z-player-photo')}<strong>${zEsc(p.name)}</strong>${nfl?(owner?`<em>${zEsc(owner.name)}</em>`:''):`<small>${zEsc(p.slot)} · ${zEsc(p.nfl)}</small>`}</button>`;
}
...
function zFormation(rows,top,nfl){
 ...
 return `<div class="z-formation ${top?'z-top':'z-bottom'}"><div class="z-backs">${[0,1].map(i=>`<div class="z-back-slot">${backs[i]?zBubble(backs[i],nfl):''}</div>`).join('')}</div><div class="z-qb">${qb?zBubble(qb,nfl):''}</div><div class="z-receivers">${wings.map(p=>zBubble(p,nfl)).join('')}</div>${zLine()}${nfl?'':`<div class="z-field-specialists">${rows.filter(p=>['K','DEF'].includes(p.slot)).map(p=>zBubble(p,false,true)).join('')}</div>`}</div>`;
}
...
function renderMatchups(){
 ...
 app.innerHTML=`<div class="page z-page"><section class="z-page-heading wrap"><div><div class="eyebrow">THE LEAGUE, ON THE FIELD</div><h1>Matchups</h1><p>Every player. Every owner. One field.</p></div><a class="z-text-link" href="#season2026" data-z-go="season2026">Read the Week 1 recap ↗</a></section><div class="wrap">${zWeekButtons()}<div class="z-match-layout"><aside class="z-sidebar"><label for="zMatchMode">Matchup view</label><select id="zMatchMode"><option value="zynfl">ZYNFL matchups</option><option value="nfl">NFL matchups</option></select><label class="z-match-mobile-label" for="zMatchSelect">Choose a matchup</label><select class="z-match-select" id="zMatchSelect" aria-label="Choose a matchup"></select><div id="zMatchList" class="z-match-list" aria-label="Choose a matchup"></div></aside><section class="z-match-main" aria-label="Selected matchup"><div class="z-field-note">Tap a player for points, projection & game log. Bigger bubbles mean more fantasy points.</div><div id="zField"></div></section></div></div></div>`;
...
function zDrawMatchups(){
 ...
 const select=document.getElementById('zMatchSelect');select.innerHTML=games.map((g,i)=>`<option value="${i}">${zEsc(g[0])} vs. ${zEsc(g[1])}</option>`).join('');select.value=zState.match;select.onchange=()=>{zState.match=Number(select.value);zDrawMatchups();};
 ...
 field.innerHTML=`<div class="z-field-scroll" aria-label="Football field"><div class="z-field">${zSideHeader(keys[0],nfl?g[2]:zTotal(keys[0]),nfl)}${zFormation(roster[0],true,nfl)}<div class="z-midfield"><span>ZYNFL</span><small>WEEK 01</small></div>${zFormation(roster[1],false,nfl)}${zSideHeader(keys[1],nfl?g[3]:zTotal(keys[1]),nfl,true)}</div></div>${nfl?`<div class="z-depth-note">Depth charts · September 15, 2026.<div>${keys.map(k=>`<a href="${Z26.nflTeams[k].source}" target="_blank" rel="noopener noreferrer">${k} depth chart ↗</a>`).join(' · ')}</div></div><details class="z-reserves"><summary>More rostered players in this NFL game</summary><div class="z-bench-grid">${Z26.players.filter(p=>p.owner&&keys.includes(p.nfl)&&!roster.flat().some(x=>x.id===p.id)).map(p=>zBubble(p,true,true)).join('')||'<p>No additional players.</p>'}</div></details><details class="z-reserves"><summary>Owner colors</summary><div class="z-legend">${Object.entries(Z26.teams).map(([o,t])=>`<span><i style="background:${t.color}"></i>${zEsc(t.name)}</span>`).join('')}</div></details>`:keys.map((k,i)=>`<details class="z-reserves"><summary>${zEsc(k)}’s bench · ${roster[i].filter(p=>p.slot==='BN').length} players</summary><div class="z-bench-grid">${roster[i].filter(p=>p.slot==='BN').map(p=>zBubble(p,false,true)).join('')}</div></details>`).join('')}`;
...
function zOpenPlayer(id){
 ...
 d.innerHTML=`<button class="z-dialog-close" aria-label="Close player details">×</button><div class="z-dialog-heading">${zPhoto(p.headshot,p.name)}<div><span class="z-eyebrow">${zEsc(p.position)} · ${zEsc(p.nfl)}</span><h2 id="zDialogTitle">${zEsc(p.name)}</h2>${p.owner?`<p>${zEsc(Z26.teams[p.owner].name)}</p>`:''}</div></div><div class="z-detail-stats"><div><strong>${zPts(zScore(p))}</strong><small>${p.status==='projected'?'Assumed points (projection)':'Fantasy points'}</small></div><div><strong>${zPts(p.projection)}</strong><small>Projected Points</small></div><div><strong>${p.points==null||p.projection==null?'—':(p.points-p.projection>0?'+':'')+(p.points-p.projection).toFixed(2)}</strong><small>vs. projection</small></div></div>${Z26.injuryNotes[p.name]?`<p class="z-injury">${zEsc(Z26.injuryNotes[p.name])}</p>`:''}<h3>2026 game log</h3><div class="z-table-scroll"><table class="z-log"><thead><tr><th>Week</th><th>Opponent</th><th>Lineup</th><th>Points</th><th>Projection</th><th>Status</th></tr></thead><tbody><tr><td>1</td><td>${opponent||'—'}</td><td>${zEsc(p.owner?p.slot:'—')}</td><td>${zPts(zScore(p))}</td><td>${zPts(p.projection)}</td><td>${p.status==='final'?'Final':p.status==='projected'?'Assumed':'—'}</td></tr></tbody></table></div>`;
...
function zDrawRankings(){
 ...
 document.getElementById('zRankingContent').innerHTML=order?`${jack?'<p class="z-muted">Jack’s Week 1 Power Rankings.</p>':''}<ol class="z-ranking-list">${order.map((o,i)=>`<li><strong class="z-rank">${String(i+1).padStart(2,'0')}</strong>${zPhoto(teamPhoto(o),o)}<div><h3>${zEsc(Z26.teams[o].name)}</h3><small>${zEsc(o)} · ${zRecord(o)} · ${zPts(zTotal(o))} Week 1 points</small><p>${zEsc(jack?Z26.jackRankReasons[o]:Z26.rankReasons[o])}</p>${zScheduleNote(o)?`<p class="z-muted">${zEsc(zScheduleNote(o))}</p>`:''}</div></li>`).join('')}</ol>`:'';
